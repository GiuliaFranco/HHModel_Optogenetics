...
#define nrn_init _nrn_init__hhCN
#define _nrn_initial _nrn_initial__hhCN
#define nrn_cur _nrn_cur__hhCN
#define _nrn_current _nrn_current__hhCN
#define nrn_jacob _nrn_jacob__hhCN
#define nrn_state _nrn_state__hhCN
#define _net_receive _net_receive__hhCN 
#define func func__hhCN 
#define states states__hhCN 
...
#define etah etah_hhCN
#define etam etam_hhCN
#define etan etan_hhCN
...
#define TK TK_hhCN
 double TK = 400;
#define gamNa gamNa_hhCN
 double gamNa = 10;
#define gamK gamK_hhCN
 double gamK = 10;
#define tau tau_hhCN
 double tau = 1;
#define wNa2 wNa2_hhCN
 double wNa2 = 200;
#define wK2 wK2_hhCN
 double wK2 = 150;
...
static void nrn_alloc(Prop* _prop) {
	...
	double *_p; Datum *_ppvar;
 	_p = nrn_prop_data_alloc(_mechtype, 41, _prop);
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
...
#endif
  hoc_register_prop_size(_mechtype, 41, 7);
  ...
  hoc_register_dparam_semantics(_mechtype, 6, "cvodeieq");
 	...
 	ivoc_help("help ?1 hhCN /Users/giulia/Desktop/Tesi/VagrantNeuro/TutorialsNeuron_and_Brian/DAcomparison/HH/Neuron/x86_64/hhCN.mod\n");
...
/*CVODE*/
 ...
 {
   double _lflag , _ln0 , _lm0 , _lh0 , _ls0 ;
 func ( _threadargscomma_ v ) ;
   ...
   _lm0 = m ;
   while ( _lflag  == 0.0 ) {
     Dm = 2.0 * ( am * ( 1.0 - m ) - bm * m + etam ( _threadargscomma_ am , bm ) ) ;
     if ( m < 0.0  || m > 1.0 ) {
       m = _lm0 ;
       }
     else {
 ...
 func ( _threadargscomma_ v ) ;
   ...
   _lm0 = m ;
   while ( _lflag  == 0.0 ) {
      m = m + (1. - exp(dt*(( 2.0 )*( ( ( am )*( ( ( - 1.0 ) ) ) - ( bm )*( 1.0 ) ) ))))*(- ( ( 2.0 )*( ( ( am )*( ( 1.0 ) ) + etam ( _threadargscomma_ am , bm ) ) ) ) / ( ( 2.0 )*( ( ( am )*( ( ( - 1.0 ) ) ) - ( bm )*( 1.0 ) ) ) ) - m) ;
     if ( m < 0.0  || m > 1.0 ) {
       m = _lm0 ;
       }
     else {
...
static int  func (  double _lv ) {
   double _lq10 ;
  as = 0.00005 * exp ( - ( _lv + 85. ) / 30. ) ;
   bs = 0.00051 / ( exp ( - 0.3 * ( _lv + 17. ) ) + 1.0 ) ;
 ...
 v=_v;
{
  ...
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 76 in file hhCN.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 }  }}
...
static void _initlists() {
 ...
 _slist1[5] = &(pK) - _p;  _dlist1[5] = &(DpK) - _p;
 _slist1[6] = &(qNa) - _p;  _dlist1[6] = &(DqNa) - _p;
 _slist1[7] = &(pNa) - _p;  _dlist1[7] = &(DpNa) - _p;
_first = 0;
}

#if NMODL_TEXT
static const char* nmodl_filename = "/Users/giulia/Desktop/Tesi/VagrantNeuro/TutorialsNeuron_and_Brian/DAcomparison/HH/Neuron/hhCN.mod";
static const char* nmodl_file_text = 
