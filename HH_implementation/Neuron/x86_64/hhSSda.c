...
#define nrn_init _nrn_init__hhSSda
#define _nrn_initial _nrn_initial__hhSSda
#define nrn_cur _nrn_cur__hhSSda
#define _nrn_current _nrn_current__hhSSda
#define nrn_jacob _nrn_jacob__hhSSda
#define nrn_state _nrn_state__hhSSda
#define _net_receive _net_receive__hhSSda 
#define rates rates__hhSSda 
#define states states__hhSSda 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_hhSSda", _hoc_setdata,
 "rates_hhSSda", _hoc_rates,
 0, 0
};
 ...
 "gl_hhSSda", 0, 1e+09,
 "gkbar_hhSSda", 0, 1e+09,
 "gnabar_hhSSda", 0, 1e+09,
 0,0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
...
#endif
  hoc_register_prop_size(_mechtype, 50, 7);
  ...
  hoc_register_dparam_semantics(_mechtype, 6, "cvodeieq");
 	...
 	ivoc_help("help ?1 hhSSda /Users/giulia/Desktop/Tesi/VagrantNeuro/TutorialsNeuron_and_Brian/DAcomparison/HH/Neuron/x86_64/hhSSda.mod\n");
...
/*CVODE*/
 static int _ode_spec1 () {_reset=0;
 {
   ...
   Dmh3 = ( - 3.0 * bm - ah ) * mh3 + am * mh2 + bh * mh7 + R [ 0 ] ;
   Dmh4 = ( - 3.0 * am - bh ) * mh4 + bm * mh5 + ah * mh0 ;
   Dmh5 = ( - 2.0 * am - bm - bh ) * mh5 + 3.0 * am * mh4 + 2.0 * bm * mh6 + ah * mh1 ;
   Dmh6 = ( - am - 2.0 * bm - bh ) * mh6 + 2.0 * am * mh5 + 3.0 * bm * mh7 + ah * mh2 + R [ 1 ] ;
   Dmh7 = ( - 3.0 * bm - bh ) * mh7 + am * mh6 + ah * mh3 - R [ 1 ] - R [ 0 ] ;
   mh0 = 1.0 - mh1 - mh2 - mh3 - mh4 - mh5 - mh6 - mh7 ;
   Dn1 = ( - 3.0 * an - bn ) * n1 + 4.0 * an * n0 + 2.0 * bn * n2 ;
   Dn2 = ( - 2.0 * an - 2.0 * bn ) * n2 + 3.0 * an * n1 + 3.0 * bn * n3 ;
   Dn3 = ( - an - 3.0 * bn ) * n3 + 2.0 * an * n2 + 4.0 * bn * n4 + R [ 2 ] ;
   Dn4 = - 4.0 * bn * n4 + an * n3 - R [ 2 ] ;
   n0 = 1.0 - n1 - n2 - n3 - n4 ;
   }
 return _reset;
}
 ...
 static int states () {_reset=0;
 {
   rates ( _threadargscomma_ v ) ;
    ...
    mh3 = mh3 + (1. - exp(dt*(( ( - 3.0 * bm - ah ) )*( 1.0 ))))*(- ( ( am )*( mh2 ) + ( bh )*( mh7 ) + R[0] ) / ( ( ( ( - 3.0 )*( bm ) - ah ) )*( 1.0 ) ) - mh3) ;
    ...
    mh6 = mh6 + (1. - exp(dt*(( ( - am - 2.0 * bm - bh ) )*( 1.0 ))))*(- ( ( ( 2.0 )*( am ) )*( mh5 ) + ( ( 3.0 )*( bm ) )*( mh7 ) + ( ah )*( mh2 ) + R[1] ) / ( ( ( - am - ( 2.0 )*( bm ) - bh ) )*( 1.0 ) ) - mh6) ;
    mh7 = mh7 + (1. - exp(dt*(( ( - 3.0 * bm - bh ) )*( 1.0 ))))*(- ( ( am )*( mh6 ) + ( ah )*( mh3 ) - R[1] - R[0] ) / ( ( ( ( - 3.0 )*( bm ) - bh ) )*( 1.0 ) ) - mh7) ;
   mh0 = 1.0 - mh1 - mh2 - mh3 - mh4 - mh5 - mh6 - mh7 ;
    ...
    n3 = n3 + (1. - exp(dt*(( ( - an - 3.0 * bn ) )*( 1.0 ))))*(- ( ( ( 2.0 )*( an ) )*( n2 ) + ( ( 4.0 )*( bn ) )*( n4 ) + R[2] ) / ( ( ( - an - ( 3.0 )*( bn ) ) )*( 1.0 ) ) - n3) ;
    n4 = n4 + (1. - exp(dt*(( - 4.0 * bn )*( 1.0 ))))*(- ( ( an )*( n3 ) - R[2] ) / ( ( ( - 4.0 )*( bn ) )*( 1.0 ) ) - n4) ;
   n0 = 1.0 - n1 - n2 - n3 - n4 ;
   }
  return 0;
}
...
static int  rates (  double _lv ) {
  ...
  _lq10 = pow( 3.0 , ( ( celsius - 6.3 ) / 10.0 ) ) ;
   ...
   {int  _lii ;for ( _lii = 0 ; _lii <= 1 ; _lii ++ ) {
     R [ _lii ] = normrand ( 0.0 , 1.0 / sqrt ( NNa * dt ) ) ;
     } }
   R [ 2 ] = normrand ( 0.0 , 1.0 / sqrt ( NK * dt ) ) ;
   R [ 0 ] = R [ 0 ] * sqrt ( fabs ( ah * mh3 + bh * mh7 ) ) ;
   R [ 1 ] = R [ 1 ] * sqrt ( fabs ( am * mh6 + 3.0 * bm * mh7 ) ) ;
   R [ 2 ] = R [ 2 ] * sqrt ( fabs ( an * n3 + 4.0 * bn * n4 ) ) ;
     return 0; }
 ...
 v=_v;
{
  ...
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 78 in file hhSSda.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 }  }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/giulia/Desktop/Tesi/VagrantNeuro/TutorialsNeuron_and_Brian/DAcomparison/HH/Neuron/hhSSda.mod";
static const char* nmodl_file_text = 
