...
class HH():
	def __init__(self,Iin):
		...
		self.T = 18    # Total time of observations
		...
		#self.gbarNa = 80 # (cm^2/kΩ) Na conductance
		self.gbarK = 36 #(cm^2/kΩ) K conductance
		self.gbarl = 0.3 # (cm^2/kΩ) l conductance
		self.gbarm=0.01*self.gbarK
		'''
		self.Cm = 0.83  # Membrane Capacitance (uF/cm^2)
		self.Phi = 2.  # Temperture factor
		self.T = 50    # Total time of observations
		self.I = In_current.I(Iin,self.T)   # Injected current
		self.V_0=-60  # mV Initial value
		self.ENa = 50 # mV Na potential
		self.EK = -90 # mV K potential
		self.El = -65 # mV Leakage potential
		self.gbarNa = 58 # (cm^2/kΩ) Na conductance
		self.gbarK = 36 #(cm^2/kΩ) K conductance
		self.gbarl = 0.1 # (cm^2/kΩ) l conductance
		self.gbarm=0.01*self.gbarK
		'''
		...
		self.mode="hh"  # model selection
		self.eps = 0   # evolution of slow dynamics  (0 -> not evolving,1 -> evolving)
		self.s1_const = 1
		self.s2_const = 1
