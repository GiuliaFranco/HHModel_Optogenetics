...
double * Stoch_HH :: HHS_Guller(double *y,double *s,double t){
		...
		std::default_random_engine generator;
		std::normal_distribution<double> distribution (0.0,1.5);
		std::normal_distribution<double> distributionNa (0.0,gammaNa*TNa*(constants->A_n(V)*(1-n)+constants->B_n(V)*n));
		std::normal_distribution<double> distributionK (0.0,gammak*Tk*(constants->A_m(V)*(1-m)+constants->B_m(V)*m));

  		double epsK=distributionNa(generator);
  		double epsNa=distributionK(generator);
  		//return this->model_core(t,gNa,gK,y,
		//		(constants->D_s1(V)*(1-s_1)-constants->G_s1(V)*s_1),0);
		...
		p_K/tau,(-gammak*q_K-pow(Wk,2)*(constants->A_n(V)*(1-n)-constants->B_n(V)*n)*q_K+epsK)/tau,p_Na/tau,
		(-gammaNa*q_Na-pow(WNa,2)*(constants->A_m(V)*(1-n)-constants->B_m(V)*m)*q_Na+epsNa)/tau};
		return res;
